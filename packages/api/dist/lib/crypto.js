 ...
 */
export function hashAnswer(quizId, index, answer, salt) {
 ...
 */
export function hashAnswers(quizId, answers, salt) {
 ...
 * @returns Hex-encoded SHA256 hash
 */
export function hashCommitment(answers, nonce) {
 ...
 */
export function encryptMnemonic(mnemonic, encryptionKey) {
 ...
 */
export function decryptMnemonic(encryptedMnemonic, encryptionKey) {
    if (!encryptionKey || encryptionKey.length < 32) {
        throw new Error('Encryption key must be at least 32 characters');
    }
    try {
 ...
 * @returns Encrypted string
 */
export function encryptQuizRevealData(data, encryptionKey) {
    if (!encryptionKey || encryptionKey.length < 32) {
        throw new Error('Encryption key must be at least 32 characters');
    }
 ...
 */
export function decryptQuizRevealData(encryptedData, encryptionKey) {
    if (!encryptionKey || encryptionKey.length < 32) {
        throw new Error('Encryption key must be at least 32 characters');
    }
    try {
 ...
 * @returns Encrypted string
 */
export function encryptAttemptRevealData(data, encryptionKey) {
    if (!encryptionKey || encryptionKey.length < 32) {
        throw new Error('Encryption key must be at least 32 characters');
    }
    const jsonData = JSON.stringify(data);
 ...
 */
export function decryptAttemptRevealData(encryptedData, encryptionKey) {
    if (!encryptionKey || encryptionKey.length < 32) {
        throw new Error('Encryption key must be at least 32 characters');
    }
    try {
