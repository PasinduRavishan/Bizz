 ...
 */
async function withMempoolRetry(operation, operationName, maxRetries = 5, initialDelayMs = 3000, computer // Optional Computer instance for regtest block mining
) {
    let lastError = null;
 ...
 */
async function pollForRevealedStatus(computer, quizRev, maxAttempts = 10, delayMs = 3000) {
 ...
 */
export async function distributePrizesToWinners(quizId, revealedQuizRev) {
    ...
    try {
        ...
        const { tx } = await withMempoolRetry(async () => {
        ...
        }, 'Encode distributePrizes transaction', 3, 3000, teacherComputer // Pass computer for regtest block mining
        );
        console.log(`  ✅ Transaction encoded successfully`);
        // Broadcast the transaction
        distributeTxId = await withMempoolRetry(async () => {
            return await teacherComputer.broadcast(tx);
        }, 'Broadcast distributePrizes transaction', 3, 3000, teacherComputer // Pass computer for regtest block mining
        );
    ...
    for (let i = 0; i < quiz.winners.length; i++) {
        ...
        try {
            ...
            const { tx: paymentTx, effect: paymentEffect } = await withMempoolRetry(async () => {
            ...
            }, `Create Payment contract for winner ${i + 1}`, 3, 3000, teacherComputer);
            console.log(`    ✅ Payment transaction encoded`);
            // Broadcast Payment contract creation
            const paymentTxId = await withMempoolRetry(async () => {
                return await teacherComputer.broadcast(paymentTx);
            }, `Broadcast Payment contract for winner ${i + 1}`, 3, 3000, teacherComputer);
 ...
 */
export async function payEntryFeesToTeacher(quizId) {
    ...
    const payments = quiz.attempts.map(attempt => ({
        ...
        status: 'accounted' // Marked as accounted (funds in contracts)
    }));
    return {
 ...
 */
export async function processCompletePayments(quizId, revealedQuizRev) {
 ...
 */
async function refreshBalances(quizId) {
    console.log(`\n🔄 Refreshing balances...`);
    const quiz = await prisma.quiz.findUnique({
        where: { id: quizId },
        include: {
 ...
 */
export async function claimPayment(userId, paymentRev, moduleSpecifier) {
    ...
    try {
        ...
        console.log(`  Satoshis locked: ${payment._satoshis}`);
        ...
        }, 'Encode claim call', 3, 2000, computer);
        const claimTxId = await withMempoolRetry(async () => {
            return await computer.broadcast(tx);
        }, 'Broadcast claim', 3, 2000, computer);
        console.log(`  ✅ Claim transaction broadcast: ${claimTxId}`);
        // Wait for blockchain confirmation
        await new Promise(resolve => setTimeout(resolve, 3000));
        // Query for latest Payment revision
        const [latestPaymentRev] = await computer.query({ ids: [payment._id] });
...
//# sourceMappingURL=payment-distribution.js.map