 ...
 */
export async function fundUserWallet(userId, amount) {
    // Get user's wallet
    ...
    const fundingComputer = new Computer({
        chain: (process.env.NEXT_PUBLIC_BITCOIN_COMPUTER_CHAIN || 'LTC'),
        network: process.env.NEXT_PUBLIC_BITCOIN_COMPUTER_NETWORK,
        url: process.env.NEXT_PUBLIC_BITCOIN_COMPUTER_URL || 'https://rltc.node.bitcoincomputer.io',
    ...
    const txId = await fundingComputer.send(userAddress, String(amountInBTC));
    // Update user balance
    await getUserBalance(userId);
 ...
 */
export async function initializeUserWallet(userId) {
    const encryptionKey = process.env.WALLET_ENCRYPTION_KEY;
    if (!encryptionKey) {
        throw new Error('WALLET_ENCRYPTION_KEY not configured');
    }
