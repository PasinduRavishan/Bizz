 ...
 */
export async function ensureWalletHasUTXOs(userId, minAmount = 100000, skipCheck = false) {
    ...
    const { balance, utxos } = balanceData;
    ...
    throw new Error(`Wallet has ${balance} sats but all are locked in contracts. ` +
 ...
 */
export async function checkWalletReady(userId) {
    const computer = await getUserWallet(userId);
    const balanceData = await computer.getBalance();
    const { balance, utxos } = balanceData;
    return {
