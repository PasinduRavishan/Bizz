...
export class TeacherRevealService {
   ...
   */
  static async revealAndGrade(
    teacherComputer: Computer,
    params: { teacherId: string; quizId: string; quizModuleSpec?: string }
  ) {
    try {
      ...
      const correctAnswers = JSON.parse(quiz.correctAnswers as string) as string[]
      ...
      for (const attempt of quiz.attempts) {
        if (attempt.status === 'COMMITTED' && attempt.answers) {
          const studentAnswers = JSON.parse(attempt.answers as string) as string[]
          const gradeResult = QuizScoring.gradeAttempt(
            studentAnswers,
            correctAnswers,
            quiz.passThreshold
          )
  ...
  static async verifyAttempt(
    ...
    params: { attemptId: string; score: number; passed: boolean; attemptModuleSpec?: string }
  ) {
    try {
