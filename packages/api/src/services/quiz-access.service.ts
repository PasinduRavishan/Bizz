...
export class QuizAccessService {
   ...
   */
  static async prepareAccess(teacherComputer: Computer, params: { quizId: string }) {
    try {
      const { quizId } = params
   ...
   */
  static async completeAccess(
    studentComputer: Computer,
    teacherComputer: Computer,
    params: { studentId: string; quizId: string; attemptId: string; partialExecTx: string }
  ) {
    try {
      ...
      const entryPayment = paymentEffect.res as any
      ...
      await prisma.quizAttempt.update({
        where: { id: attempt.id },
        data: {
          quizTokenRev: (studentQuizToken as any)._rev,
          status: 'OWNED'
        }
      })
      ...
      return {
        success: true,
        attemptId: (validatedAttempt as any)._id,
        attemptRev: (validatedAttempt as any)._rev,
        execTxId,
        redeemTxId
      }
    } catch (error) {
