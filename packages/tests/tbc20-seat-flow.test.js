 ...
 * Bitcoin Computer Quiz Platform - Fungible Seat Tokens + On-Demand Attempts
 *
 * PHASE 1: Quiz Creation & Seat Token Minting (TBC20)
 *   - Teacher creates quiz with prize pool
 *   - Teacher mints 100 fungible SEAT tokens (single UTXO)
 *
 * PHASE 2: Seat Purchase (TBC20 + EXEC)
 *   - Students buy seat tokens via exec pattern
 *   - TBC20 transfer splits: teacher's supply reduces, student gets new UTXO
 *   - Entry fee paid atomically
 *
 * PHASE 3: Attempt Creation (On-Demand)
 *   - Students create QuizAttempt after owning seat
...
describe('🎓 TBC20 SEAT TOKEN FLOW - Fungible Tokens + On-Demand Attempts', function () {
  ...
  const totalSeats = 100n  // TBC20: Teacher mints 100 fungible seats

  // Quiz data
  let quiz, quizId, answerHashes

  // Seat token data (TBC20)
  let quizSeats  // Teacher's fungible seat token UTXO
  let student1Seat, student2Seat, student3Seat  // Student seat UTXOs

  // Attempt data (created on-demand)
  ...
  describe('Phase 1: Quiz Creation & Seat Token Purchase (TBC20 + EXEC)', () => {
    ...
    it('should mint fungible seat tokens (TBC20)', async function () {
      console.log('\n  🪙 Teacher minting fungible SEAT tokens...')

      await mineBlockFromRPCClient(teacherComputer)

      const teacherBalanceBefore = (await teacherComputer.getBalance()).balance

      // Mint 100 SEAT tokens in ONE UTXO (TBC20 fungible token)
      const { tx, effect } = await withRetry(
        () => teacherComputer.encode({
      ...
      console.log(`    ✅ Minted ${totalSeats} SEAT tokens in ONE UTXO`)
      console.log(`    ✅ Token ID: ${quizSeats._id.substring(0, 20)}...`)
      console.log(`    ✅ Symbol: ${quizSeats.symbol}`)
      console.log(`    ✅ Amount: ${quizSeats.amount}`)
      console.log(`    ✅ Quiz Ref: ${quizSeats.quizRef.substring(0, 20)}...`)
      console.log(`    ✅ Owner: ${quizSeats._owners[0].substring(0, 20)}...`)
    ...
    it('should allow student 1 to purchase seat token via exec (TBC20)', async function () {
      console.log('\n  💰 Student 1 purchasing SEAT token via EXEC (TBC20 pattern)...')
      ...
      console.log(`\n    📊 Before purchase:`)
      console.log(`      Teacher's SEAT balance: ${quizSeats.amount}`)
      ...
      const { tx: partialExecTx } = await withRetry(
        () => teacherComputer.encode({
          exp: `${SeatAccess} SeatAccess.exec(seatToken, entryFeePayment)`,
          env: {
            seatToken: quizSeats._rev,
            entryFeePayment: paymentMock._rev  // Mock payment revision
          },
          mocks: { entryFeePayment: paymentMock },  // Tell encoder it's mocked
          mod: seatAccessModuleSpec,
          sighashType: SIGHASH_SINGLE | SIGHASH_ANYONECANPAY,
          inputIndex: 0,
          fund: false,  // Student will fund
          sign: true    // Teacher signs their input (seatToken)
        }),
      ...
      // Update output to ensure student receives the seat token
      ...
      // STEP 5: Query updated states (TBC20 creates TWO UTXOs)
      console.log('\n     Step 5: Verifying TBC20 exec results...')

      // Query teacher's remaining seat tokens
      const [latestQuizSeatsRev] = await teacherComputer.query({ ids: [quizSeats._id] })
      quizSeats = await teacherComputer.sync(latestQuizSeatsRev)

      // Query student's NEW seat token UTXO
      const student1SeatRevs = await student1Computer.query({
        publicKey: student1PubKey,
        order: 'DESC'
      })

      // Find the newly created seat token for student 1
      for (const rev of student1SeatRevs) {
        const obj = await student1Computer.sync(rev)
        if (obj.symbol === 'SEAT' && obj.amount === 1n) {
          student1Seat = obj
          break
        }
      }
      ...
      console.log(`      Teacher's SEAT balance: ${quizSeats.amount} (reduced by 1)`)
      console.log(`      Student 1 SEAT token: ${student1Seat ? student1Seat.amount : 'NOT FOUND'}`)
      console.log(`      Student 1 SEAT owner: ${student1Seat ? (student1Seat._owners[0] === student1PubKey ? 'Student 1 ✅' : 'NOT STUDENT ❌') : 'N/A'}`)
      ...
      expect(quizSeats.amount).to.equal(99n) // Teacher has 99 left
      expect(student1Seat).to.not.be.undefined
      expect(student1Seat.amount).to.equal(1n) // Student has 1
      expect(student1Seat._owners[0]).to.equal(student1PubKey)
      expect(student1Seat.symbol).to.equal('SEAT')
    ...
    it('should allow student 1 to redeem seat and create quiz attempt', async function () {
      ...
      const { tx: attemptTx, effect: attemptEffect } = await withRetry(
        () => student1Computer.encode({
          ...
          exp: `new QuizAttempt("${student1PubKey}", "${quizId}", "", BigInt(${entryFee}), "${teacherPubKey}")`
        }),
        'Create quiz attempt',
        student1Computer
      )
      ...
      expect(attempt1.quizRef).to.equal(quizId)
    })

    it('should allow student 1 to submit answers to their attempt', async function () {
    ...
    it('should allow students 2 and 3 to purchase seats, create attempts, and submit', async function () {
      console.log('\n  💰 Students 2 & 3 purchasing SEAT tokens via EXEC (TBC20)...')

      // ========== STUDENT 2 PURCHASE SEAT ==========
      console.log('\n  👨‍🎓 Student 2 Purchase Flow:')

      await mineBlockFromRPCClient(teacherComputer)

      // Sync latest quizSeats state
      const [latestSeatsRev] = await teacherComputer.query({ ids: [quizSeats._id] })
      quizSeats = await teacherComputer.sync(latestSeatsRev)
      console.log(`    📊 Teacher's SEAT balance before: ${quizSeats.amount}`)
      ...
      // Query student 2's seat token
      const student2SeatRevs = await student2Computer.query({
        publicKey: student2PubKey,
        order: 'DESC'
      })
      for (const rev of student2SeatRevs) {
        const obj = await student2Computer.sync(rev)
        if (obj.symbol === 'SEAT' && obj.amount === 1n) {
          student2Seat = obj
          break
        }
      ...
      // ========== STUDENT 3 PURCHASE SEAT ==========
      console.log('\n  👨‍🎓 Student 3 Purchase Flow:')

      await mineBlockFromRPCClient(teacherComputer)

      // Sync latest quizSeats state
      const [latestSeats2Rev] = await teacherComputer.query({ ids: [quizSeats._id] })
      quizSeats = await teacherComputer.sync(latestSeats2Rev)
      console.log(`    📊 Teacher's SEAT balance before: ${quizSeats.amount}`)
      ...
      // Query student 3's seat token
      const student3SeatRevs = await student3Computer.query({
        publicKey: student3PubKey,
        order: 'DESC'
      })
      for (const rev of student3SeatRevs) {
        const obj = await student3Computer.sync(rev)
        if (obj.symbol === 'SEAT' && obj.amount === 1n) {
          student3Seat = obj
          break
        }
      ...
      // Verify final state
      const [finalSeatsRev] = await teacherComputer.query({ ids: [quizSeats._id] })
      quizSeats = await teacherComputer.sync(finalSeatsRev)
      console.log(`\n    📊 Final SEAT balance: ${quizSeats.amount} (started with 100, sold 3)`)

      console.log(`\n    ✅ Both students purchased seats, created attempts, and submitted successfully`)
      expect(quizSeats.amount).to.equal(97n)
