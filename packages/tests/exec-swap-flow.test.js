/**
 * EXEC + SWAP FLOW INTEGRATION TEST
 * Bitcoin Computer Quiz Platform - Entry Fee Collection via Exec Pattern
 *
 * Tests the complete flow with exec and swap patterns:
 * PHASE 1: Quiz Creation & Attempt Purchase (EXEC)
 *   - Teacher creates quiz with prize pool
 *   - Teacher creates empty attempts (teacher-owned)
 *   - Students buy attempts by paying entry fee (exec pattern)
 *   - Students submit answers after purchase
 *
 * PHASE 2: Teacher Reveal & Scoring
 *   - Teacher reveals correct answers
 *   - Teacher scores attempts from commitments
 *   - NO student reveal phase
 *
 * PHASE 3: Prize Distribution (SWAP)
 *   - Winners create AnswerProof contracts
 *   - Winners swap AnswerProof for prize (swap pattern)
 *   - Losers already paid entry fee in Phase 1
 *
 * Based on Bitcoin Computer exec/swap patterns from monorepo docs
 */
...
describe('🎓 EXEC + SWAP FLOW - Entry Fee Collection & Prize Distribution', function () {
  ...
  let quizModuleSpec, attemptModuleSpec, accessModuleSpec, proofModuleSpec, swapModuleSpec
  ...
  before(async function () {
    ...
    accessModuleSpec = await withRetry(
      () => teacherComputer.deploy(`export ${AttemptAccess}`),
      'Deploy AttemptAccess module',
      teacherComputer,
      10
    )
    console.log(`    ✅ AttemptAccess module deployed`)
    await mineBlockFromRPCClient(teacherComputer)
    await sleep(3000)
    await mineBlockFromRPCClient(teacherComputer)
    await sleep(3000)
  ...
  describe('Phase 1: Quiz Creation & Attempt Purchase (EXEC)', () => {
    ...
    it('should create empty attempts owned by teacher', async function () {
      console.log('\n  📝 Teacher creating empty attempts...')

      await mineBlockFromRPCClient(teacherComputer)

      // Create 3 empty attempts (teacher-owned)
      const { tx: tx1, effect: effect1 } = await withRetry(
        () => teacherComputer.encode({
          mod: attemptModuleSpec,
          exp: `new QuizAttempt("${teacherPubKey}", "${quizId}", "", BigInt(${entryFee}), "${teacherPubKey}")`
        }),
        'Encode attempt 1',
        teacherComputer
      )
      await teacherComputer.broadcast(tx1)
      attempt1 = effect1.res
      ...
      attempt2 = effect2.res

      await mineBlockFromRPCClient(teacherComputer)

      const { tx: tx3, effect: effect3 } = await withRetry(
        () => teacherComputer.encode({
          mod: attemptModuleSpec,
          exp: `new QuizAttempt("${teacherPubKey}", "${quizId}", "", BigInt(${entryFee}), "${teacherPubKey}")`
        }),
      ...
      await teacherComputer.broadcast(tx3)
      attempt3 = effect3.res

      await sleep(3000)

      console.log(`    ✅ Attempt 1: ${attempt1._id.substring(0, 20)}... (status: ${attempt1.status})`)
      console.log(`    ✅ Attempt 2: ${attempt2._id.substring(0, 20)}... (status: ${attempt2.status})`)
      console.log(`    ✅ Attempt 3: ${attempt3._id.substring(0, 20)}... (status: ${attempt3.status})`)

      expect(attempt1.status).to.equal('available')
      expect(attempt1._owners[0]).to.equal(teacherPubKey)
      expect(attempt2.status).to.equal('available')
      expect(attempt3.status).to.equal('available')
    })
